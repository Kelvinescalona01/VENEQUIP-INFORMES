...
// NOTA DE SEGURIDAD: nunca hardcodees claves de API en el código fuente,
// menos aún en un repositorio público — cualquiera puede leerla y usarla
// a tu costa. La clave anterior quedó expuesta en el historial de Git;
// si aún es válida, revócala/regenérala desde Google AI Studio ya mismo.
// Ahora se lee en tiempo de compilación desde una variable de entorno
// de Vite (debe empezar con VITE_ para exponerse al cliente).
...
export async function callGeminiRestDirect(
  ...
  // `process.env` no existe en el navegador (solo en Node/servidor), por lo
  // que esa comprobación nunca funcionaba aquí. Vite expone las variables
  // públicas a través de `import.meta.env`.
  const apiKey = GEMINI_API_KEY_DEFAULT;
  if (!apiKey) {
    throw new Error(
      'No hay una clave de API de Gemini configurada para el navegador. ' +
      'Define VITE_GEMINI_API_KEY en tu archivo .env, o asegúrate de que ' +
      'el proxy del servidor (GEMINI_API_KEY) esté disponible.'
    );
  }
